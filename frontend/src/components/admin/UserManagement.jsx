...
import { Users, Shield, Search, Filter, ShieldCheck, MoreHorizontal, X } from 'lucide-react';
                            ...
                            <div key={user.id} className={`flex items-center justify-between p-4 px-5 hover:bg-slate-50/50 transition-colors group ${index !== filteredUsers.length - 1 ? 'border-b border-slate-50' : ''}`}>
                                ...
                                <div className="flex items-center gap-8">
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 pl-3 pr-2.5 py-1.5 bg-white border border-slate-200 text-slate-500 hover:text-secondary hover:border-secondary hover:bg-secondary/5 rounded-lg text-xs font-bold transition-all shadow-sm"
                                    >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white max-w-md w-full rounded-2xl shadow-2xl border border-slate-100 overflow-hidden transform animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest">User Information</h3>
                            <button
                        ...
                        <div className="p-6">
                            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
                                <div className={`w-16 h-16 flex flex-col items-center justify-center rounded-xl font-bold text-xl shadow-sm ${getRoleAvatarStyle(selectedUser.role)}`}>
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                        ...
                                        <span className={`text-[10px] font-black px-2 py-0.5 rounded border uppercase tracking-widest bg-white ${getRoleBadge(selectedUser.role)}`}>
                                            {selectedUser.role}
                                        </span>
                                    </div>
                                    <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full ${selectedUser.status === 'Now' ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                                            <span className="text-xs font-bold text-slate-700">{selectedUser.status === 'Now' ? 'Online' : 'Offline'}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    ...
                                    <p className="text-xs font-medium text-slate-700">{selectedUser.status === 'Now' ? 'Currently active in dashboard' : `Last seen ${selectedUser.status}`}</p>
                                </div>
                            </div>
                        </div>
                    </div>
