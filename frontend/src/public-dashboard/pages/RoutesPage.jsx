...
export default function RoutesPage() {
    ...
    const routes = {
        fastest: [
            { id: 'f1', from: locationZone.name, to: destinationName, time: "6 Min", crowdBadge: "Very Light", 
              steps: [
                  { desc: `Start at ${locationZone.name}`, type: 'start' }, 
                  { desc: "Proceed via Garden Path", type: 'path' }, 
                  { desc: "Take Escaltor C up", type: 'transit' }, 
                  { desc: `Arrive at ${destinationName}`, type: 'end' }
              ] 
            },
            { id: 'f2', from: locationZone.name, to: "Ticket Counter Area", time: "2 Min", crowdBadge: "Normal", 
              steps: [
                  { desc: `Start at ${locationZone.name}`, type: 'start' }, 
                  { desc: "Walk straight 50m", type: 'path' }, 
                  { desc: "Arrive at Ticket Counter", type: 'end' }
              ] 
            }
        ],
        safest: [
            { id: 's1', from: locationZone.name, to: destinationName, time: "9 Min", crowdBadge: "Empty", 
              steps: [
                  { desc: `Start at ${locationZone.name}`, type: 'start' }, 
                  { desc: "Bypass Main Entrance crowd via West Alley", type: 'path' }, 
                  { desc: "Use Service Elevator B", type: 'transit' }, 
                  { desc: `Arrive at ${destinationName}`, type: 'end' }
              ] 
            }
        ],
        accessible: [
            { id: 'a1', from: locationZone.name, to: "Food Court Zone", time: "12 Min", crowdBadge: "Normal", 
              steps: [
                  { desc: `Start at ${locationZone.name}`, type: 'start' }, 
                  { desc: "Follow blue painted ramp line", type: 'path' }, 
                  { desc: "Use Wheelchair Lift A", type: 'transit' }, 
                  { desc: "Arrive at Food Court Zone Level 2", type: 'end' }
              ] 
            }
        ]
    };
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h1 className="text-3xl font-black text-[#002868] tracking-tight">Safe Route Suggestions</h1>
                <div className="flex bg-white rounded-xl p-1 shadow-sm border border-gray-200">
                    <button onClick={() => { setActiveTab('fastest'); setSelectedRoute(null); }} className={`px-5 py-2.5 font-bold text-sm rounded-lg transition-all ${activeTab === 'fastest' ? 'bg-[#002868] text-white shadow-md scale-[1.02]' : 'text-gray-500 hover:bg-gray-50'}`}>Fastest</button>
                    <button onClick={() => { setActiveTab('safest'); setSelectedRoute(null); }} className={`px-5 py-2.5 font-bold text-sm rounded-lg transition-all ${activeTab === 'safest' ? 'bg-[#00AEEF] text-white shadow-md scale-[1.02]' : 'text-gray-500 hover:bg-gray-50'}`}>Safest</button>
                    <button onClick={() => { setActiveTab('accessible'); setSelectedRoute(null); }} className={`px-5 py-2.5 font-bold text-sm rounded-lg transition-all ${activeTab === 'accessible' ? 'bg-indigo-600 text-white shadow-md scale-[1.02]' : 'text-gray-500 hover:bg-gray-50'}`}>Accessible</button>
                </div>
            </div>
